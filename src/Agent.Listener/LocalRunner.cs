...
namespace Microsoft.VisualStudio.Services.Agent.Listener
{
    ...
    public sealed class LocalRunner : AgentService, ILocalRunner
    {
        ...
        public async Task<int> LocalRunAsync(CommandSettings command, CancellationToken token)
        {
                ...
                string[] ymlFiles =
                    ...
                    .Where((string filePath) =>
                    {
                        return filePath.EndsWith(".yml", IOUtil.FilePathStringComparison) ||
                            filePath.EndsWith(".yaml", IOUtil.FilePathStringComparison);
                    })
                    .ToArray();
                if (ymlFiles.Length > 1)
                {
                    throw new Exception($"More than one YAML file exists in the current directory. Specify which file to use via the '{Constants.Agent.CommandLine.Args.Yml}' command line argument.");
                }
            }

            if (string.IsNullOrEmpty(ymlFile))
            {
                throw new Exception($"Unable to find a YAML file in the current directory. Specify which file to use via the '{Constants.Agent.CommandLine.Args.Yml}' command line argument.");
            }
            ...
            string url = command.GetUrl(suppressPrompt: true);
            if (string.IsNullOrEmpty(url))
            {
                if (!TestAllTasksCached(process, token))
                {
                    url = command.GetUrl(suppressPrompt: false);
                }
            }
